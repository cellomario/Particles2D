...
void DumpPopulation(struct Population p, int t)
{
   ...
   */	
   ...
   sprintf(fname,"output/Population%4.4d.dmp\0",t);
   dump = fopen(fname,"w");
   if ( dump == NULL ) {
...
void ParticleStats(struct Population p, int t)
{
   ...
   if ( t <= 0 ) stats = fopen("output/Population.sta","w"); 
     else stats = fopen("output/Population.sta","a"); // append new data
   if ( stats == NULL ) {
   ...
   if ( forces == NULL ) {
     ...
     for ( t=0; t < mxiter; t++ ) { // iteration
       ...
       for ( i=0; i < pp->np; i++ ) {
         ...
         #pragma omp parallel for reduction(+:f_0_i,f_1_i)
			for ( j=0; j < pp->np; j++ ) {
				if ( j != i ) {
	            double f[2];
               struct particle p2;
...
void ParticleScreen(struct i2dGrid* pgrid, struct Population pp, int step) 
{
	...
	sprintf(name,"output/stage%3.3d\0",step);
